...
def process_image(image_path: str, resize_rate: float) -> Image.Image:
...
def combine_images_in_grid(
    ...
    with ProcessPoolExecutor() as executor:
        images = list(executor.map(
            process_image, image_paths, [resize_rate] * len(image_paths))
        )
    ...
    combined_image = Image.new(
        "RGB", (grid_width, grid_height), color=(255, 255, 255)
    )
